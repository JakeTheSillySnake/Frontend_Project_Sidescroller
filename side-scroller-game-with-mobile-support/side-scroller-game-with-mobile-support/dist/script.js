    ...
    constructor() {
     ...
     // mobile touch support
      ...
      window.addEventListener("touchmove", e => {
        ...
        if (lives === 0 && this.keys.indexOf("swipe left") > -1) restartGame();
      })
      window.addEventListener("touchend", e => {
        this.keys.splice(this.keys.indexOf("swipe up"), 1); this.keys.splice(this.keys.indexOf("swipe left"), 1); this.keys.splice(this.keys.indexOf("swipe right"), 1);
      })
...
canvas.requestFullscreen().catch(err => {
  ...
  ctx.font = "40px Georgia";
  ctx.fillStyle = "black";
  ctx.fillText("Press Enter or swipe left", canvas.width/2 + 2, 52);
  ctx.fillStyle = "white";
  ctx.fillText("Press Enter or swipe left", canvas.width/2, 50);
